...
import { reportHealthCheck } from './utils/analytics';
...
export class Datasource extends DataSourceWithBackend<InfinityQuery, InfinityOptions> {
  ...
  async fetchData(query: InfinityQuery, options: DataQueryRequest<InfinityQuery>): Promise<any> {
    // Define headers with an appropriate type
    let headers: { [key: string]: string } = {};

    // Ensure URL is present only when required for the specific query type and source
    if (
      (query.type === 'json' ||
        query.type === 'csv' ||
        query.type === 'xml' ||
    ...
    if ((query.type === 'json' ||
      ...
      query.type === 'graphql' ||
      query.type === 'groq' ||
      query.type === 'tsv') && query.source === 'url' && query.url) {
      return axios.get(query.url, { headers }).then((response) => response.data);
    } else {
      throw new Error('Unsupported query source or missing URL.');
    }
  }
  ...
  private getResults(options: DataQueryRequest<InfinityQuery>, result?: DataQueryResponse): Promise<DataQueryResponse> {
    ...
    if (result.error) {
      return Promise.resolve({ data: result.data, error: result.error, state: LoadingState.Error });
    }
    ...
    if (result.data) {
      result.data.map((d) => {
        ...
        } else if (target.type === 'google-sheets' || target.type === 'transformations') {
          promises.push(Promise.resolve(d));
        } else {
          promises.push(
